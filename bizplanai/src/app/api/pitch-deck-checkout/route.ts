...
export async function POST(req: NextRequest) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: 'usd', product_data: { name: 'AI Pitch Deck Generator', description: '12-slide investor-ready pitch deck with speaker notes.' }, unit_amount: 3900 }, quantity: 1 }],
      mode: 'payment',
      allow_promotion_codes: true,
      success_url: `${baseUrl}/pitch-deck?session_id={CHECKOUT_SESSION_ID}${planParam}`,
      cancel_url: `${baseUrl}/pitch-deck`,
      metadata: { product: 'pitch_deck', ...buildCheckoutMetadata(body) },
    });
