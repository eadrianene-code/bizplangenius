...
function subHeading(title: string): any {
  return { text: title, bold: true, fontSize: 11, color: ACCENT, margin: [0, 10, 0, 5] };
}
...
function buildPDF(plan: BusinessPlan, businessName: string) {
  ...
  content.push(
    ...
    { text: 'AI-Powered Business Plans with Real Market Research', fontSize: 7, color: LINE, alignment: 'center' },
  ...
  if (plan.marketAnalysis?.targetCustomerProfile) {
    ...
    if (tcp.painPoints?.length) {
      tcpStack.push({ text: 'Pain Points:', bold: true, fontSize: 10, color: TEXT, margin: [0, 4, 0, 3] });
  ...
  return {
    ...
    pageBreakBefore: (currentNode: any, followingNodesOnPage: any[]) => {
      /* Only break if a section heading would be completely orphaned at the very bottom
         with zero content following it on the same page */
      if (currentNode.headlineLevel === 1 && followingNodesOnPage.length <= 1) {
        return true;
      }
      return false;
    },
